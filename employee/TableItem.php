    ...
    if (pg_num_rows($result) > 0) {
      ...
      while ($row = pg_fetch_assoc($result)) {
        if ($cnt == 0) {
          ...
          <div class="table-info__food-info foodDrinkName">' . $row["name"] . "</div><div class='table-info__food-quantity'>:" . $row["quantity"] . '</div></div>';
      ...
      while ($row = pg_fetch_assoc($result)) {
        if ($cnt == 0) {
          ...
          echo '">                    <!--여기가 번호 출력하는 부분이야!!!!!!!!!-->
              ...
              <div class="table-info__drink-info foodDrinkName">' . $row["name"] . "</div><div class='table-info__food-quantity'>:" . $row["quantity"] . '</div></div>';
          ($time < ((int) substr($row["date"], 11, 2) * 3600 + (int) substr($row["date"], 14, 2) * 60 + (int) substr($row["date"], 17, 2))) ? $time : (int) substr($row["date"], 11, 2) * 3600 + (int) substr($row["date"], 14, 2) * 60 + (int) substr($row["date"], 17, 2);
        } else {
          $cnt++;
          $count++;
          ...
          echo '">                <!--여기가 번호 출력하는 부분이야!!!!!!!!!-->
          <div class="table-info__drink-num foodDrinkNum">' . $count . '</div>
          <div class="table-info__drink-info foodDrinkName">' . $row["name"] . "</div><div class='table-info__food-quantity'>:" . $row["quantity"] . '</div>
          </div>';
          ($time < ((int) substr($row["date"], 11, 2) * 3600 + (int) substr($row["date"], 14, 2) * 60 + (int) substr($row["date"], 17, 2))) ? $time : (int) substr($row["date"], 11, 2) * 3600 + (int) substr($row["date"], 14, 2) * 60 + (int) substr($row["date"], 17, 2);
        }
      }
